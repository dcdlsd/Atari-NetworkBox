...
// alive at once.  Eight entries were too few and caused dropped SYN packets
// ("no flow slot") during a transfer.
...
// Return a free entry for a new TCP flow.  Connections that completed their
// FIN/RST handshake may be reused immediately: retaining them for two extra
// seconds only starves a busy FTP client.  A still-active flow is never evicted.
...
static err_t wifi_output_trace(struct netif *netif, struct pbuf *p,
    ...
    if (ethernet_active && w5500_lwip_take_tx_timeout()) {
        // A W5500 command that never completes used to freeze the whole
        // NetworkBox. Restarting the Pico also resets the Ethernet module.
...
static bool nat_send_tcp_packet(struct pbuf *input, struct netif *wifi_netif) {
    ...
    if (!flow && (flags & 0x02)) { // First SYN for this TCP connection.
        uint32_t now = to_ms_since_boot(get_absolute_time());
    ...
    if (!flow) {
        printf("NAT TCP: no flow slot for %u -> %u\n", client_port, server_port);
...
int main()
{
    ...
    sleep_ms(30); // anti-rebond au démarrage
    const bool force_wifi = !gpio_get(MODE_BUTTON_PIN);
    button_long_press_armed = !force_wifi;

    // Le RJ45 est volontairement prioritaire : s'il est branché au démarrage,
    // le réseau Atari passe par le W5500. Un bouton maintenu force le Wi-Fi.
    if (force_wifi) {
        printf("Mode button held: forcing Wi-Fi.\n");
    } else {
        try_start_w5500();
    }
    ...
    if (!ethernet_active && have_credentials) {
        cyw43_arch_enable_sta_mode();
    ...
    if (!ethernet_active && !station_connected && !start_setup_portal()) return 1;
    ...
    if (ethernet_active) {
        ...
        printf("W5500 set as default lwIP route\n");
    }
    ...
    // Last-resort recovery is needed only for the W5500/SPI path.  The Wi-Fi
    // stack may legitimately spend more than five seconds in a call while it
    // waits for radio/network activity, so enabling this watchdog in Wi-Fi
    // mode would cause false restarts.
    const bool w5500_watchdog_enabled = ethernet_active;
    if (w5500_watchdog_enabled) {
        watchdog_enable(5000, true);
        watchdog_update();
        printf("W5500 watchdog enabled (5 s).\n");
    }

    uint32_t reported_uart_overflow = 0;
    while (true) {
        if (w5500_watchdog_enabled) watchdog_update();
        poll_w5500_frames();
        ...
        if (now - last_status >= 1000) {
            if (ethernet_active) {
                printf(ip4_addr_isany_val(ethernet_netif.ip_addr)
                           ? "NetworkBox active - W5500 waiting for DHCP\n"
                           : "NetworkBox active - W5500 OK\n");
            } else {
                printf(station_connected ? "NetworkBox active - Wi-Fi OK\n"
                                         : "NetworkBox active - Wi-Fi setup mode\n");
            }
            last_status = now;
        }
        tight_loop_contents();

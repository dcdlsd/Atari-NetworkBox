...
static bool tx_timeout_seen = false;
...
int32_t send_lwip(uint8_t sn, uint8_t *buf, uint16_t len)
{
    ...
    while (getSn_CR(sn)) {
        if (time_reached(deadline)) {
            printf("W5500 TX command timeout\n");
            tx_timeout_seen = true;
            return -2;
        }
        tight_loop_contents();
    }
    ...
    while (1)
    {
        ...
        if (time_reached(deadline)) {
            printf("W5500 TX completion timeout\n");
            tx_timeout_seen = true;
            return -2;
        }
        tight_loop_contents();
...
bool w5500_lwip_take_tx_timeout(void)
{
    bool result = tx_timeout_seen;
    tx_timeout_seen = false;
    return result;
}
...
err_t netif_output(struct netif *netif, struct pbuf *p)
{
    ...
    return send_len < 0 ? ERR_TIMEOUT : ERR_OK;
}
